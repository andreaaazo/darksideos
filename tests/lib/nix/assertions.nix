...
{lib}: rec {
  ...
  in
    ...
    then ''
      ...
      echo "  Expected: ${builtins.toJSON "all assertions pass"}"
      echo "  Actual:   ${builtins.toJSON "all assertions passed"}"
      echo "  Severity: high"
      echo "  Rationale: Eval assertions must preserve the declared configuration contract."
      touch $out
    ''
    else ''
      echo "============================================"
